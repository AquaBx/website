<script lang="ts">
	...
	let cvHeight = $state(31.5);
	let cvSummary = $state(snapshot.profile?.summary || "");
	let avatarSize = $state(80);
	let headerBg = $state("#0f172a");
	let headerText = $state("#ffffff");
	...
	let mainTypContent = $derived.by(() => {
		...
		profileDictEntries.push(`summary: ${toTypstValue(cvSummary)}`);
...
#cv_template(
  ...
  height: ${cvHeight}cm,
  header_bg: rgb("${headerBg}"),
  header_text: rgb("${headerText}"),
  avatar_size: ${avatarSize}pt
)
`.trim();
	});
...
<div class="flex flex-col gap-12 py-12 print:p-0">
	...
	>
		...
		<div class="lg:w-1/3 w-full print:hidden flex flex-col gap-6">
			...
			<!-- CV Layout & Content Card -->
			<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col gap-4">
				<h2 class="text-lg font-bold text-slate-900 border-b border-slate-100 pb-3 mb-2 flex items-center gap-2">
					<Icon src={SlidersHorizontal} class="w-5 h-5 text-indigo-500 shrink-0" />
					Ajustement du CV
				</h2>
				...
				<div class="flex flex-col gap-1.5">
					<div class="flex justify-between text-xs font-semibold text-slate-600">
						<span>Hauteur de page</span>
						<span class="text-indigo-600 font-mono">{cvHeight.toFixed(1)} cm</span>
					</div>
					<input 
						type="range" 
						min="28" 
				...
				<div class="flex flex-col gap-1.5">
					<div class="flex justify-between text-xs font-semibold text-slate-600">
						<span>Taille de la photo (pt)</span>
						<span class="text-indigo-600 font-mono">{avatarSize} pt</span>
					</div>
					<input 
						type="range" 
						min="50" 
				...
				<div class="flex gap-4">
					<div class="flex-1 flex flex-col gap-1.5">
						<span class="text-xs font-semibold text-slate-600">Fond en-tête</span>
						<div class="flex items-center gap-2">
							<input 
								type="color" 
								bind:value={headerBg}
								class="w-10 h-8 p-0 border border-slate-200 rounded cursor-pointer"
							/>
							<span class="text-xs font-mono text-slate-500 uppercase">{headerBg}</span>
						</div>
					</div>

					<div class="flex-1 flex flex-col gap-1.5">
						<span class="text-xs font-semibold text-slate-600">Texte en-tête</span>
						<div class="flex items-center gap-2">
							<input 
								type="color" 
								...
								class="w-10 h-8 p-0 border border-slate-200 rounded cursor-pointer"
							/>
							<span class="text-xs font-mono text-slate-500 uppercase">{headerText}</span>
						</div>
					</div>
				</div>

				<div class="flex flex-col gap-1.5">
					<span class="text-xs font-semibold text-slate-600">Description du CV</span>
					<textarea 
						bind:value={cvSummary}
						rows="4"
						class="w-full text-sm p-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50 text-slate-800 resize-y leading-relaxed font-sans"
						placeholder="Écrivez votre description de profil..."
					></textarea>
				</div>
			</div>
