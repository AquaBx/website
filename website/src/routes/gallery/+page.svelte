<script lang="ts">
	...
	import { Carousel } from "@website/uikit";
...
<div class="flex flex-col gap-12 py-12">
	<Title title={m.gallery()} />

	<div class="flex-wrap justify-between flex gap-4 text-white">
			...
			{#if post.sources.length > 1}
				...
				>
					...
					{#snippet previous(onclick)}
						<button
							{onclick}
							class="z-10 rounded-full p-2 bg-black absolute left-5 top-1/2 -translate-y-1/2"
						>
					...
					{#snippet next(onclick)}
						<button
							{onclick}
							class="z-10 rounded-full p-2 bg-black absolute right-5 top-1/2 -translate-y-1/2"
						>
